...
interface FooInterface {
    ...
    // REEB NOTE - Streams not implemented so this part should not go to documentation
...
class HowToCustomize
{
    public function showSamples(): void
    {
        ...
        try {
            ...
            $session = $store->openSession();
            try {
                // REEB NOTE: we do not have streams in PHP !!!
                //region customize_1_1_1
                $totalStreamedResultsSize = 0;

                $session->query(Employee::class)
                    ->addAfterStreamExecutedListener(function($result) use (&$totalStreamedResultsSize) {
